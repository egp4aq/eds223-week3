...
# This would be finding all of the highest points that are not in canterbury

# Using st_filter() to subset
c_height2 <- nz_height %>%
  st_filter(y=canterbury, predicate = st_intersects)

# Creating logical vector based on st_intersects()
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury)
# this gives us 1 if it's true and nothing if it's empty. So, to get a list of T/F we can look at the length of the value
...
nz_height_logical2 <- st_is_within_distance(nz_height, canterbury,
                      dist = units::set_units(1000, 'km'), # set distance
                      sparse = FALSE) # return logical vector instead

c_height4 <- nz_height[nz_height_logical2,]
```

## Spatial Joins

```{r}
# by default, st_join uses a left join
...
# st_is_within_distance goes row by row in the first data set and checks which points in the second data set are within 20 m of that point

head(cycle_hire_join)
```

```{r}
# Building some checks to diagnose the output
...
```{r}
# combines all of the western states
us_west <- us_states[us_states$REGION == "West", ]
us_west_union <- st_union(us_west)

# adds texas to that combined western state chunk
...
```{r}
# going back and doing the buffer on our nz data
nz_height_buffer <- st_buffer(nz_height, dist = 1000000)

# same as if we had done st_filter with st_is_within_distance
c_height5 <- nz_height_buffer %>%
  st_filter(y=canterbury, .predicate = st_intersects)

# proving to ourselves that the filter worked because they have the same number of rows
